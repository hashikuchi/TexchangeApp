...
//Rare exceptions to indicate a bug in the _texchange code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_texchange = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_texchange]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.texchange.module"
